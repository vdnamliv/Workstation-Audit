...
func New(store storage.Store, cfg model.Config) *Server { return &Server{Store: store, Cfg: cfg} }
...
func (s *Server) routes(mux *http.ServeMux) {
    ...
    mux.HandleFunc("/api/policy/active", s.handlePolicyActive)
    mux.HandleFunc("/api/policy/history", s.handlePolicyHistory)
    mux.HandleFunc("/api/policy/save", s.handlePolicySave)
    mux.HandleFunc("/api/policy/activate", s.handlePolicyActivate)
...
func (s *Server) handleResults(w http.ResponseWriter, r *http.Request) {
    ...
    if from != "" { if ts, ok := parseDate(from); ok { fromTS = &ts } }
    if to != "" { if ts, ok := parseDate(to); ok { t := ts + 86400; toTS = &t } }
...
func (s *Server) handlePolicyActive(w http.ResponseWriter, r *http.Request) {
    ap, _ := s.Store.LoadActivePolicy("windows")
    if ap == nil { http.Error(w, "no policy", 404); return }
...
func (s *Server) handlePolicySave(w http.ResponseWriter, r *http.Request) {
    ...
    if !s.allowAdmin(r) { http.Error(w, "forbidden", 403); return }
    ...
    nextV := 1
    if cur != nil { nextV = cur.Version + 1 }
    cfgBlob, _ := json.Marshal(struct {
        Version  int                      `json:"version"`
        Policies []map[string]interface{} `json:"policies"`
    }{Version: nextV, Policies: rules})
    hash := policy.JSONHash(cfgBlob)
    if err := s.Store.InsertPolicyVersion("win_baseline", "windows", nextV, cfgBlob, hash, in.YAML); err != nil { http.Error(w, "db", 500); return }
    if err := s.Store.SetActivePolicy("windows", "win_baseline", nextV); err != nil { http.Error(w, "db", 500); return }
...
func (s *Server) handlePolicyActivate(w http.ResponseWriter, r *http.Request) {
    ...
    if err := s.Store.SetActivePolicy("windows", in.PolicyID, in.Version); err != nil { http.Error(w, "db", 500); return }
    _ = json.NewEncoder(w).Encode(map[string]any{"ok": true})
}

func (s *Server) handleReloadPolicies(w http.ResponseWriter, r *http.Request) {
    if !s.allowAdmin(r) { http.Error(w, "forbidden", http.StatusForbidden); return }
...
func (s *Server) allowAdmin(r *http.Request) bool {
    if s.Cfg.AdminKey != "" {
        ...
        return subtleCTCompare([]byte(key), []byte(s.Cfg.AdminKey)) == 1
    }
...
func parseDate(s string) (int64, bool) { t, err := time.Parse("2006-01-02", s); if err != nil { return 0, false }; return t.Unix(), true }
